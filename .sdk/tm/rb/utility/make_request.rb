...
module StephenKingUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = StephenKingResponse.new({})
    ...
    if fetch_err
      ...
      response = StephenKingResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = StephenKingResponse.new(fetched)
    else
