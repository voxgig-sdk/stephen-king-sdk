...
from projectname_sdk.core.control import StephenKingControl
...
class StephenKingContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, StephenKingSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, StephenKingResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, StephenKingResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return StephenKingError(code, msg, self)
