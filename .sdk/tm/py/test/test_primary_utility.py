...
from projectname_sdk import StephenKingSDK
from projectname_sdk.core.spec import StephenKingSpec
from projectname_sdk.core.result import StephenKingResult
from projectname_sdk.core.response import StephenKingResponse
from projectname_sdk.core.operation import StephenKingOperation
from projectname_sdk.core.error import StephenKingError
from projectname_sdk.core import helpers
from projectname_sdk.feature.base_feature import StephenKingBaseFeature
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return StephenKingError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_feature_init_basic(self):
        init_client = StephenKingSDK.test(None, None)
        ...
        class TestInitFeature(StephenKingBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(StephenKingBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = StephenKingSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = StephenKingSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = StephenKingOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = StephenKingSDK.test(None, setup_opts)
