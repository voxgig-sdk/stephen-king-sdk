...
[MCP](https://modelcontextprotocol.io) server exposing the StephenKing SDK as
two agent tools — `stephen-king_list` and `stephen-king_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/stephen-king-mcp)
make build

# 2. Provide credentials via the environment
export STEPHEN_KING_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user stephen-king \
  -- /absolute/path/to/stephen-king-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./stephen-king-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// stephen-king_list: first page of records
{ "entity": "book" }
{ "entity": "book", "query": { } }

// stephen-king_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/stephen-king-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user stephen-king \
     -- "$PWD"/dist/*/stephen-king-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `stephen-king_list` and `stephen-king_load` tools now appear
   in new sessions. Ask the agent to *"list book using stephen-king"*
   and it calls `stephen-king_list` with `{"entity":"book"}`.
...
```sh
export STEPHEN_KING_APIKEY=sk_live_xxx            # API key
export STEPHEN_KING_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `stephen-king_list` tool
...
### Call the `stephen-king_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `stephen-king_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `stephen-king_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `STEPHEN_KING_APIKEY` | API key sent with every request. |
| `STEPHEN_KING_BASE` | Optional override of the API base URL. |
...
```sh
./stephen-king-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"stephen-king_load","arguments":{"entity":"book","query":{"id":1}}}}'
```
