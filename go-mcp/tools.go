...
type Args struct {
	Entity string         `json:"entity" jsonschema:"book | short | villain"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.StephenKingSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "stephen-king_list",
		Description: "List records from StephenKing. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "stephen-king_load",
		Description: "Load a single record from StephenKing. " +
...
func runOp(client *sdk.StephenKingSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.StephenKingSDK, name string) (sdk.StephenKingEntity, error) {
	switch strings.ToLower(name) {
	case "book":
		return client.Book(nil), nil
	case "short":
		return client.Short(nil), nil
	case "villain":
		return client.Villain(nil), nil
