...
class TestVillainEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Villain(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Villain(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set STEPHEN_KING_TEST_VILLAIN_ENTID JSON to run live")
        client = setup["client"]
        ...
        villain_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(villain_ref01_data_dt0_loaded))
...
def _villain_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("STEPHEN_KING_TEST_VILLAIN_ENTID"))
