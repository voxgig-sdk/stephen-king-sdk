...
boru-driven command-line client **and** interactive REPL for the StephenKing
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/stephen-king-cli)
make build

# 2. See usage (words, entities, env vars)
./stephen-king-cli --help

# 3. Provide credentials once, via the environment
export STEPHEN_KING_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./stephen-king-cli list book
./stephen-king-cli load 1 book            # {id:1} shorthand
./stephen-king-cli load '{id:1}' book       # explicit match map
./stephen-king-cli list short

# 5. Override the API base URL for a single call
STEPHEN_KING_BASE=https://api.example.com ./stephen-king-cli list book

# 6. No arguments -> interactive REPL
./stephen-king-cli
stephen-king> list book
stephen-king> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/stephen-king-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/stephen-king-cli list book
   ```
...
```sh
./stephen-king-cli load 1 book          # scalar shorthand for {id:1}
./stephen-king-cli load '{id:1}' book     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `stephen-king>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/stephen-king-cli`. |
...
(`./stephen-king-cli <expr>`) and interactively (the REPL), and expressions compose the
