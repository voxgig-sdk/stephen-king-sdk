// Command StephenKing-cli is a CLI + REPL for the StephenKing SDK,
...
const prompt = "stephen-king"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "book short villain"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      STEPHEN_KING_APIKEY (api key), STEPHEN_KING_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: STEPHEN_KING_APIKEY carries the API key and
	// STEPHEN_KING_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("STEPHEN_KING_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("STEPHEN_KING_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewStephenKingSDK(opts)
