# StephenKing Ruby SDK



The Ruby SDK for the StephenKing API — an entity-oriented client using idiomatic Ruby conventions.
...
raise a `StephenKingError` (a `StandardError` subclass), so wrap
...
| `id` | `Integer` | Unique identifier for the short story |
...
```ruby
# list returns an Array of Short records (raises on error).
shorts = client.Short.list
```
...
| `id` | `Integer` | Unique identifier for the villain |
...
```ruby
# list returns an Array of Villain records (raises on error).
...
```
rb/
├── StephenKing_sdk.rb       -- Main SDK module
