...
class VillainEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set STEPHEN_KING_TEST_VILLAIN_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    villain_ref01_data_dt0_load_result = Helpers.to_map(villain_ref01_data_dt0_loaded.respond_to?(:data_get) ? villain_ref01_data_dt0_loaded.data_get : villain_ref01_data_dt0_loaded)
...
def villain_basic_setup(extra)
  ...
  entid_env_raw = ENV["STEPHEN_KING_TEST_VILLAIN_ENTID"]
