...
require_relative 'StephenKing_types'
...
class StephenKingSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = StephenKingHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = StephenKingHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, StephenKingFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    StephenKingUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = StephenKingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = StephenKingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = StephenKingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = StephenKingHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = StephenKingError.new(
        "graphql_error", "StephenKingSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = StephenKingSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
