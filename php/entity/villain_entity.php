...
// StephenKing SDK Villain entity
...
class VillainEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new VillainEntity($this->_client, $opts);
    }

    /**
     * @param Villain|array $args Villain data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = StephenKingHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Villain|array The current Villain data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Villain fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = StephenKingHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Villain fields).
     */
     ...
     *
     * @param VillainLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed VillainLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Villain|array The loaded Villain as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = StephenKingHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *
     * @param VillainListMatch|array|null $reqmatch Match filter (any subset
     *   of Villain fields) as an assoc-array; VillainListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Villain[]|array A list of Villain items as assoc-arrays at
