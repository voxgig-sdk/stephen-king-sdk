...
class StephenKingSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $utility = new StephenKingUtility();
        ...
        $feature_opts = StephenKingHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = StephenKingHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!StephenKingFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, StephenKingFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return StephenKingUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = StephenKingHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = StephenKingHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = StephenKingHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new StephenKingError($op . "_allow",
                "StephenKingSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = StephenKingHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new StephenKingError("graphql_error",
                "StephenKingSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Short($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_short === null) {
                $this->_short = new ShortEntity($this, null);
            }
            return $this->_short;
        }
        return new ShortEntity($this, $data);
    }
    ...
    public function Villain($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_villain === null) {
                $this->_villain = new VillainEntity($this, null);
            }
            return $this->_villain;
        }
        return new VillainEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new StephenKingSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
