...
// StephenKing SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = StephenKingOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(StephenKingOmni::structfix($now, Struct::undef()))
                        !== json_encode(StephenKingOmni::structfix($val, Struct::undef()))) {
