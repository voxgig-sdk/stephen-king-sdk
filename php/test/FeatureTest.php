...
class FtCtrl extends StephenKingControl
{
...
class FtClient
{
    ...
    private StephenKingUtility $utility;

    public function __construct(StephenKingUtility $utility, array $options)
    {
    ...
    public function get_utility(): StephenKingUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (StephenKingContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public StephenKingContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (StephenKingContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = StephenKingConfig::make_feature($name);
    ...
    private static function buildUrl(StephenKingSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof StephenKingError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new StephenKingResponse($fetched);
        }
    ...
    private function fail(StephenKingContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (StephenKingContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
