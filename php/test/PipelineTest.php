...
{
    private static function utility(): StephenKingUtility
    {
        return new StephenKingUtility();
    }
    ...
    private static function ctx(array $over = []): StephenKingContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new StephenKingContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new StephenKingOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof StephenKingError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new StephenKingOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = StephenKingMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new StephenKingOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = StephenKingMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new StephenKingOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = StephenKingMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new StephenKingSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = StephenKingMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new StephenKingResponse([]),
            'result' => new StephenKingResult([]),
        ]);
        [, $err] = StephenKingMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new StephenKingSpec([]),
            'response' => null,
            'result' => new StephenKingResult([]),
        ]);
        [, $err] = StephenKingMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new StephenKingSpec(['step' => 's']),
            'response' => new StephenKingResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new StephenKingResult([]),
        ]);
        [, $err] = StephenKingMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new StephenKingSpec(['step' => 's']),
            'response' => new StephenKingResponse(self::resp(200, ['v' => 1])),
            'result' => new StephenKingResult([]),
        ]);
        [, $err] = StephenKingMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new StephenKingSpec(['step' => 's']),
            'response' => new StephenKingResponse(self::resp(200, ['v' => 2])),
            'result' => new StephenKingResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        StephenKingMakeResponse::call($ctx);
    ...
    {
        $preset = new StephenKingResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new StephenKingSpec([]),
            'response' => new StephenKingResponse([]),
            'result' => new StephenKingResult([]),
        ...
        [$got, $err] = StephenKingMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new StephenKingResult([])]);
        [, $err] = StephenKingMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new StephenKingSpec([]), 'result' => null]);
        [, $err] = StephenKingMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new StephenKingSpec(['step' => 's']),
            'result' => new StephenKingResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new StephenKingOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = StephenKingMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new StephenKingSpec(['step' => 's']),
            'result' => new StephenKingResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new StephenKingOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = StephenKingMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new StephenKingResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new StephenKingSpec([]),
            'result' => new StephenKingResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = StephenKingMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = StephenKingMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new StephenKingError('boom', 'boom');
        $utility->fetcher = function (StephenKingContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new StephenKingSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = StephenKingMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (StephenKingContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = StephenKingMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(StephenKingResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        StephenKingMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (StephenKingContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new StephenKingSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = StephenKingMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new StephenKingResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new StephenKingSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = StephenKingMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = StephenKingMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = StephenKingMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new StephenKingResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, StephenKingDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new StephenKingResult(['ok' => false])]);
        $this->expectException(StephenKingError::class);
        StephenKingDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new StephenKingResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', StephenKingMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        StephenKingMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new StephenKingBaseFeature();
        $b = new StephenKingBaseFeature();
        StephenKingFeatureAdd::call($ctx, $a);
        StephenKingFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): StephenKingBaseFeature
    {
        $f = new StephenKingBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        StephenKingFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): StephenKingContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new StephenKingSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = StephenKingPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        StephenKingPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new StephenKingResponse(['headers' => null]),
            'result' => new StephenKingResult([]),
        ]);
        StephenKingResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        StephenKingResultBody::call($ctx);
