...
class VillainEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Villain(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Villain(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set STEPHEN_KING_TEST_VILLAIN_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $villain_ref01_data_dt0_load_result = Helpers::to_map(is_object($villain_ref01_data_dt0_loaded) && method_exists($villain_ref01_data_dt0_loaded, 'data_get') ? $villain_ref01_data_dt0_loaded->data_get() : $villain_ref01_data_dt0_loaded);
...
function villain_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("STEPHEN_KING_TEST_VILLAIN_ENTID");
