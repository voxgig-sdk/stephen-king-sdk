...
//     (\`new StephenKingSDK(...)\` / \`StephenKingSDK.test(...)\` become
//     \`StephenKingSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"book":{"test01":{"id":"test01"}},"short":{"test01":{"id":"test01"}},"villain":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new StephenKingSDK(...)\` and any
// \`StephenKingSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+StephenKingSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/StephenKingSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
