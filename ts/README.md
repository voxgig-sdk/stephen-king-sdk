# StephenKing TypeScript SDK



The TypeScript SDK for the StephenKing API — a type-safe, entity-oriented client with full async/await support.
...
| `tester(testopts?, sdkopts?)` | `StephenKingSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `StephenKingSDK.test(testopts?, sdkopts?)` | `StephenKingSDK` | Create a test-mode client. |
...
| `client` | `client(): StephenKingSDK` | Return the parent SDK client. |
...
```ts
const short = await client.Short().load({ id: 1 })
```
...
```ts
const villain = await client.Villain().load({ id: 1 })
```
...
stephen-king/
├── src/
│   ├── StephenKingSDK.ts        # Main SDK class
