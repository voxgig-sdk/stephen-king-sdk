...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("STEPHEN_KING_TEST_LIVE") == "TRUE" ||
		os.Getenv("STEPHEN_KING_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("STEPHEN_KING_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.StephenKingSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.StephenKingError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.StephenKingSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.StephenKingError{Msg: msg, Code: code}
}
