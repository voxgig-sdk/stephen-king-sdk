...
import (
	...
	omni "github.com/voxgig-sdk/stephen-king-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.StephenKingError); is {
				out := map[string]any{
					"name":    "StephenKingError",
...
func omniCtx(arg any, client *sdk.StephenKingSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.StephenKingSDK); is {
			client = live
