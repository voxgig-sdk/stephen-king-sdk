...
func (sdk *StephenKingSDK) OptionsMap() map[string]any {
...
func (sdk *StephenKingSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *StephenKingSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *StephenKingSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *StephenKingSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *StephenKingSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *StephenKingSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("StephenKingSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *StephenKingSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *StephenKingSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("StephenKingSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
