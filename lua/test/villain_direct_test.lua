...
describe("VillainDirect", function()
  it("should direct-list-villain", function()
    local setup = villain_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-villain", setup.live and "live" or "unit")
  ...
  it("should direct-load-villain", function()
    local setup = villain_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-villain", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function villain_direct_setup(mockres)
