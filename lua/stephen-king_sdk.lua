...
local StephenKingSDK = {}
StephenKingSDK.__index = StephenKingSDK
...
StephenKingSDK._make_feature = _make_feature


function StephenKingSDK.new(options)
  local self = setmetatable({}, StephenKingSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function StephenKingSDK:options_map()
...
function StephenKingSDK:get_utility()
  return Utility.copy(self._utility)
end


function StephenKingSDK:get_root_ctx()
  return self._rootctx
end


function StephenKingSDK:prepare(fetchargs)
...
function StephenKingSDK:direct(fetchargs)
...
-- by passing it.
function StephenKingSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function StephenKingSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "StephenKingSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function StephenKingSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = StephenKingSDK.new(sdkopts)
  sdk.mode = "test"
