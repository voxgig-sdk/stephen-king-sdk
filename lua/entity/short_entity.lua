-- StephenKing SDK Short entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ShortEntity = {}
ShortEntity.__index = ShortEntity


function ShortEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ShortEntity)
  self._name = "short"
  self._client = client
...
function ShortEntity:get_name()
  return self._name
end


function ShortEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ShortEntity.new(self._client, opts)
end
...
function ShortEntity:mark_deleted()
  self._deleted = true
end


function ShortEntity:deleted()
  return true == self._deleted
end


function ShortEntity:data_set(args)
  if args ~= nil then
...
function ShortEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ShortEntity:match_set(args)
  if args ~= nil then
...
function ShortEntity:match_get()
...
function ShortEntity:stream(action, args, callopts)
...
function ShortEntity:_run_op(ctx, post_done)
  local utility = self._utility
