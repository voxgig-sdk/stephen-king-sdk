...
local VillainEntity = {}
VillainEntity.__index = VillainEntity


function VillainEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, VillainEntity)
  self._name = "villain"
  self._client = client
...
function VillainEntity:get_name()
  return self._name
end


function VillainEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return VillainEntity.new(self._client, opts)
end
...
function VillainEntity:mark_deleted()
  self._deleted = true
end


function VillainEntity:deleted()
  return true == self._deleted
end


function VillainEntity:data_set(args)
  if args ~= nil then
...
function VillainEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function VillainEntity:match_set(args)
  if args ~= nil then
...
function VillainEntity:match_get()
...
function VillainEntity:stream(action, args, callopts)
...
function VillainEntity:_run_op(ctx, post_done)
  local utility = self._utility
